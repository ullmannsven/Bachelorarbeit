...
def fom_compute_output(fom, mu):
    ...
    gradient_FOM = fom.output_d_mu(fom.parameters.parse(mu), return_array=True, use_adjoint=True)
...
def update_kernel_model(X_train, y_train, grad_y_train, kernel, TR_parameters, M_cholesky_old=None, M_inv_cholesky_old=None):
    """
    ...
    if M_cholesky_old is None and M_inv_cholesky_old is None: 
        K_np1 = kernel.eval(X_train, X_train) #this is already with the new point, just for test reasons atm 
        B = compute_gradient_matrix_B(kernel, X_train, K_np1, TR_parameters)
        C = compute_gradient_matrix_C(kernel, X_train, K_np1, TR_parameters)

        M = np.zeros(((dim+1)*n, (dim+1)*n))
        ...
        y = np.append(y_train, np.atleast_2d(np.ravel(grad_y_train)).T, axis=0)
        weigths = np.linalg.solve(M, y)

    else:
       
        k_np1_tilde = kernel.eval(X_train[:-1], X_train[-1,:])
        block_4 = compute_block_4(kernel, X_train, TR_parameters)
        block_5 = compute_block_5(kernel, X_train, TR_parameters)
        block_6, block_7 = compute_block_6_and_7(kernel, X_train, TR_parameters)

        #This is for test reasons only, remove later
        # K_test = M_cholesky_old @ M_cholesky_old.T 
        # K_new_test = np.zeros((n*(dim+1), n*(dim+1))) 
        # K_new_test[:K_test.shape[0], :K_test.shape[1]] = K_test
        # K_new_test[K_test.shape[0]:K_test.shape[0] + dim, :n-1] = block_5.T
        # K_new_test[:n-1, K_test.shape[1]:K_test.shape[1] + dim] = block_5
        # K_new_test[K_test.shape[0]:K_test.shape[0] + dim, (n-1):(n-1)*(dim+1)] = block_6.T
        # K_new_test[(n-1):(n-1)*(dim+1), K_test.shape[1]:K_test.shape[1] + dim] = block_6
        # K_new_test[-1, :(n-1)] = k_np1_tilde[:,0].T
        # K_new_test[:(n-1), -1] = k_np1_tilde[:,0]
        # K_new_test[-1, (n-1):-1] = block_4[0,:]
        # K_new_test[(n-1):-1, -1] = block_4[0,:].T
        # K_new_test[K_test.shape[0]:K_test.shape[0] + dim,  K_test.shape[1]:K_test.shape[1] + dim] = block_7
        # K_new_test[-1,-1] = 1

        D, C = compute_increment_cholesky(X_train, kernel, k_np1_tilde, block_4, block_5, block_6, block_7, M_cholesky_old, M_inv_cholesky_old)

        #Permutations in the target values 
        y = np.concatenate((y_train[:-1], np.atleast_2d(np.ravel(grad_y_train)).T, np.atleast_2d(y_train[-1])), axis=0)
        weigths = C.T @ C @ y

        #Permutations in weigths
        weigths[n-1], weigths[-1] = weigths[-1], weigths[n-1]
        
    grad_aux = lambda x, i: - 2*(TR_parameters['kernel_width']**2)*(X_train[i,:] - x[0,:])*np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(X_train[i,:] - x[0,:])**2))
    kernel_model = lambda x: kernel.eval(x,X_train) @ weigths[:n] + sum(np.dot(weigths[(n + i*dim):(n+(i+1)*dim)][:,0], grad_aux(x,i)) for i in range(n))
        
    return kernel_model, D, C

def compute_increment_cholesky(X_train, kernel, k_np1_tilde, block_4, block_5, block_6, block_7, M_cholesky_old, M_inv_cholesky_old): 
    n = len(X_train[:,0]) 
    dim = len(X_train[0,:])
    x_new = X_train[-1,:]

    K_np1_tilde = np.concatenate((block_5, block_6), axis=0)
    k_np1 = np.concatenate((k_np1_tilde, block_4.T), axis=0)

    D_np1_tilde = np.zeros(((n-1)*(dim+1), dim))
    #TODO is this also possible without loop? check 
    for i in range(dim):
        #TODO auch hier ist wieder rückwärtseinsetzen möglich. Rausfinden ob np.linalg.solve das automatisch macht, ich denke aber ja
        D_np1_tilde[:,i] = np.linalg.solve(M_cholesky_old, K_np1_tilde[:,i])

    #TODO: this does not work currently 
    D_np1_bar = np.linalg.cholesky(block_7 - D_np1_tilde.T @ D_np1_tilde)

    #To compute d_np1 we can make use of the fact that it can be solved via a simple backtracking 
    D_help = np.zeros((n*(dim+1) - 1, n*(dim+1) - 1))
    D_help[:(n-1)*(dim+1), :(n-1)*(dim+1)] = M_cholesky_old
    D_help[(n-1)*(dim+1):(n-1)*(dim+1) + dim, :(n-1)*(dim+1)] = D_np1_tilde.T 
    D_help[-dim:, -dim:] = D_np1_bar

    d_np1 = np.linalg.solve(D_help, k_np1)
    d_nn = np.sqrt(kernel.eval(x_new, x_new) - np.dot(d_np1.T, d_np1))

    #Build together the new cholesky decomp of M_np1 
    D = np.zeros((n*(dim+1), n*(dim+1)))
    D[:n*(dim+1) - 1, :n*(dim+1) -1] = D_help
    #D[:(n-1)*(dim+1),:(n-1)*(dim+1)] = M_cholesky_old
    #D[(n-1)*(dim+1):(n-1)*(dim+1) + dim, :(n-1)*(dim+1)] = D_np1_tilde.T
    D[-1,-1] = d_nn 
    D[-1, :-1] = d_np1.T 
    #D[(n-1)*(dim+1):(n-1)*(dim+1) + dim, (n-1)*(dim+1):(n-1)*(dim+1) + dim] = D_np1_bar

    #Compute the inverse also incrementally 
    C_np1_bar = np.linalg.inv(D_np1_bar)
    c_nn = 1 / d_nn

    C_np1_tilde = np.zeros((dim, (n-1)*(dim+1)))
    rhs =  - D_np1_tilde.T @ M_inv_cholesky_old
    for i in range((n-1)*(dim+1)):
        C_np1_tilde[:,i] = np.linalg.solve(D_np1_bar, rhs[:,i])

    C_help = np.zeros((n*(dim+1) - 1, n*(dim+1) - 1))
    C_help[:(n-1)*(dim+1), :(n-1)*(dim+1)] = M_inv_cholesky_old
    C_help[(n-1)*(dim+1):, :(n-1)*(dim+1)] = C_np1_tilde
    C_help[(n-1)*(dim+1):, (n-1)*(dim+1):] = C_np1_bar

    c_np1 = - 1/d_nn * C_help.T @ d_np1
    
    C = np.zeros((n*(dim+1), n*(dim+1)))
    C[:n*(dim+1) - 1, :n*(dim+1) - 1] = C_help 
    C[-1, :-1] = c_np1[:,0]
    C[-1,-1] = c_nn
...
def compute_RKHS_norm(kernel, fom, parameter_space):
    ...
    for i in range(amount):
        ...
        target_values[i,0], _ = fom_compute_output(fom, mu)
...
def remove_similar_points(X_train, y_train, grad_y_train, TR_parameters, iter):
    ...
    grad_y_train = np.delete(grad_y_train, (idx), axis=0)

    return X_train, y_train, grad_y_train

#TODO can be hopefully deleted in the future
def compute_gradient_matrix_B(kernel, X_train, K, TR_parameters):
    #Note: we require the squares of the kernel_width, to correspond the implementation in VKOGA
    n = len(X_train[:,0])
    dim = len(X_train[0,:])

    
    #Extend the kernel matrix K in the direction axis=1 with all the columns respectivly
    K_ext = np.concatenate((K, K), axis=1)
    K_ext = K_ext[:, [i//dim + (i%dim) * K.shape[1] for i in range(dim * K.shape[1])]]
    
    if "gauss" in kernel.name:
        B = -2*(TR_parameters['kernel_width']**2)*(X_train - X_train[:, np.newaxis]).reshape((n, dim*n)) * K_ext

    #B = np.zeros((n, dim*n))
    #for i in range(n):
    #    for j in range(n):
    #        if "gauss" in kernel.name:
    #            B[j, dim*i:(i*dim+dim)] = -2*(TR_parameters['kernel_width']**2)*(X_train[i,:] - X_train[j,:])*np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(X_train[i,:] - X_train[j,:])**2))

    return B

def compute_block_4(kernel, X_train, TR_parameters):
    n = len(X_train[:,0])
    dim = len(X_train[0,:])
    x_new = X_train[-1, :]
    ...
    for j in range(n):
        if "gauss" in kernel.name:
            B[0, dim*j:(j*dim+dim)] = -2*(TR_parameters['kernel_width']**2)*(X_train[j,:] - x_new)*np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(x_new - X_train[j,:])**2))

    return B

def compute_block_5(kernel, X_train, TR_parameters):
    n = len(X_train[:,0])
    dim = len(X_train[0,:])
    x_new = X_train[-1, :]
    ...
    for j in range(n-1):
        if "gauss" in kernel.name:
            B[j, :] = -2*(TR_parameters['kernel_width']**2)*(x_new - X_train[j,:])*np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(x_new - X_train[j,:])**2))

    return B

def compute_block_6_and_7(kernel, X_train, TR_parameters):
    n = len(X_train[:,0])
    dim = len(X_train[0,:])
    x_new = X_train[-1, :]
    ...
    for j in range(n):
        if "gauss" in kernel.name:
            Eps_diag = (-4*(TR_parameters['kernel_width']**4)*X_train[j,:]*(X_train[j,:] - x_new) + 2*(TR_parameters['kernel_width']**2) + 4*(TR_parameters['kernel_width']**4)*x_new*(X_train[j,:] - x_new)) \
                    * np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(x_new - X_train[j,:])**2)) 
            Eps = (-4*(TR_parameters['kernel_width']**4)*((np.atleast_2d(X_train[j,:] - x_new)).T @ np.atleast_2d(X_train[j,:] - x_new))) \
                    * np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(x_new - X_train[j,:])**2))
            np.fill_diagonal(Eps, Eps_diag)
...
def compute_gradient_matrix_C(kernel, X_train, K, TR_parameters):
    n = len(X_train[:,0])
    dim = len(X_train[0,:])
    C = np.zeros((n*dim, n*dim))

    #Extend the kernel matrix K in the direction axis=1 with all the columns respectivly
    #TODO this only for work dim=2 i guess
    #K_ext = np.concatenate((K, K), axis=1)
    #K_ext = K_ext[:, [i//dim + (i%dim) * K.shape[1] for i in range(dim * K.shape[1])]]
    #K_ext_2 = np.concatenate((K_ext, K_ext), axis=0)
    #K_ext_2 =  K_ext_2[[i//dim + (i%dim) * K_ext.shape[0] for i in range(dim * K_ext.shape[0])], :]

    for i in range(n):
        for j in range(n):
            if "gauss" in kernel.name:
                Eps_diag = (-4*(TR_parameters['kernel_width']**4)*X_train[i,:]*(X_train[i,:] - X_train[j,:]) + 2*(TR_parameters['kernel_width']**2) + 4*(TR_parameters['kernel_width']**4)*X_train[j,:]*(X_train[i,:] - X_train[j,:])) \
                        * np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(X_train[i,:] - X_train[j,:])**2)) 
                Eps = (-4*(TR_parameters['kernel_width']**4)*((np.atleast_2d(X_train[i,:] - X_train[j,:])).T @ np.atleast_2d(X_train[i,:] - X_train[j,:]))) \
                        * np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(X_train[i,:] - X_train[j,:])**2))
                np.fill_diagonal(Eps, Eps_diag)

            C[j*dim:(j+1)*dim, i*dim:(i+1)*dim] = Eps
            ...
            #        if k==l: 
            #            if "gauss" in kernel.name:
            #                Eps[k,l] = (-4*(TR_parameters['kernel_width']**4)*X_train[i,k]*(X_train[i,k] - X_train[j,k]) + 2*(TR_parameters['kernel_width']**2) + 4*(TR_parameters['kernel_width']**4)*X_train[j,k]*(X_train[i,k] - X_train[j,k])) \
            #                        * np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(X_train[i,:] - X_train[j,:])**2))
            #            else: 
            #                raise(NotImplementedError)
            #        else: 
            #            if "gauss" in kernel.name:
            #                Eps[k,l] = (-4*(TR_parameters['kernel_width']**4)*(X_train[i,k] - X_train[j,k])*(X_train[i,l] - X_train[j,l])) \
            #                        * np.exp(-(TR_parameters['kernel_width']*np.linalg.norm(X_train[i,:] - X_train[j,:])**2))
            #            else: 
            #                raise(NotImplementedError)
            #
...
def compute_gradient(kernel, mu_k, X_train, y_train, grad_y_train, TR_parameters, iteration):
    ...
    """
    X_train, y_train, grad_y_train = remove_similar_points(X_train, y_train, grad_y_train, TR_parameters, iteration)
    X_train, y_train, grad_y_train = remove_far_away_points(X_train, y_train, grad_y_train, mu_k, TR_parameters)
...
def create_training_dataset(mu_k, fom, parameter_space, X_train, y_train, grad_y_train, global_counter, iter, initial, gradient=None):
    ...
    for i in range(num_of_points_old, num_of_points):
       new_target_value, grad_target_value = fom_compute_output(fom, X_train[i,:])
       y_train = np.append(y_train, np.atleast_2d(new_target_value), axis=0)
       grad_y_train = np.append(grad_y_train, np.atleast_2d(grad_target_value).T, axis=0)
       global_counter += 1
    
    return X_train, y_train, grad_y_train, global_counter
...
def optimization_subproblem_BFGS(parameter_space, kernel_model, kernel, X_train, y_train, grad_y_train, mu_i, TR_parameters, RKHS_norm, iteration):
    ...
    gradient = compute_gradient(kernel, mu_i, X_train, y_train, grad_y_train, TR_parameters, iteration)
...
def TR_Kernel(fom, kernel, parameter_space, global_counter, RKHS_norm, TR_parameters=None):
    ...
    J_FOM_k, grad_J_FOM_k = fom_compute_output(fom, mu_k)
    times_FOM.append(time.time()-start_time)
    global_counter += 1
    J_FOM_list.append(J_FOM_k)
    grad_J_FOM_list.append(grad_J_FOM_k)
    
    X_train = mu_k
    y_train = np.zeros((1,1))
    grad_y_train = np.zeros((1,2))
    y_train[0,0] = J_FOM_k
    grad_y_train[0,:] = grad_J_FOM_k[:,0].T
    
    #Creates an initial set of training points that are in close proximitity to the starting point (currently via FD scheme, feasible in 2D)
    tic = time.time()
    X_train, y_train, grad_y_train, global_counter = create_training_dataset(mu_k, fom, parameter_space, X_train, y_train, grad_y_train, global_counter, k-1, True)
    times_FOM.append(time.time() - tic)

    kernel_model, M_cholesky_old, M_inv_cholesky_old = update_kernel_model(X_train, y_train, grad_y_train, kernel, TR_parameters)
    ...
    while k <= TR_parameters['max_iterations']:
        ...
        mu_kp1, J_AGC, j, J_kp1, gradient, success = optimization_subproblem_BFGS(parameter_space, kernel_model, kernel, X_train, y_train, grad_y_train, mu_k, TR_parameters, RKHS_norm, iteration=k-1)

        if not success: 
            tic = time.time()
            X_train, y_train, grad_y_train, global_counter = create_training_dataset(mu_kp1, fom, parameter_space, X_train, y_train, grad_y_train, global_counter, k-1, False, gradient=gradient)
            times_FOM.append(time.time() - tic)
            X_train, y_train, grad_y_train = remove_similar_points(X_train, y_train, grad_y_train, TR_parameters, k-1)
        ...
        if J_kp1 + estimator_J <= J_AGC:
            ...
            tic = time.time()
            J_FOM_kp1, grad_J_FOM_kp1 = fom_compute_output(fom, mu_kp1)
            times_FOM.append(time.time()-tic)
            global_counter += 1
            J_FOM_list.append(J_FOM_kp1)
            grad_J_FOM_list.append(grad_J_FOM_kp1)

            X_train = np.append(X_train, mu_kp1, axis=0)
            y_train = np.append(y_train, np.atleast_2d(J_FOM_kp1), axis=0)
            grad_y_train = np.append(grad_y_train, np.atleast_2d(grad_J_FOM_kp1).T, axis=0)

            X_train, y_train, grad_y_train = remove_similar_points(X_train, y_train, grad_y_train, TR_parameters, k-1)
            X_train, y_train, grad_y_train = remove_far_away_points(X_train, y_train, grad_y_train, mu_kp1, TR_parameters)
            
            print("Updating the kernel model ...\n")
            kernel_model, M_cholesky_old, M_inv_cholesky_old = update_kernel_model(X_train, y_train, grad_y_train, kernel, TR_parameters, M_cholesky_old, M_inv_cholesky_old)
        ...
        if not point_rejected:
            gradient = compute_gradient(kernel, mu_k, X_train, y_train, grad_y_train, TR_parameters, k-1)
...
TR_parameters={'radius': 0.5, 'sub_tolerance': 1e-5, 'max_iterations': 10, 'max_iterations_subproblem': 100,'starting_parameter': mu_k, 
               'max_iterations_armijo': 50, 'initial_step_armijo': 0.5, 'armijo_alpha': 1e-4, 'FOC_tolerance': 1e-8, 'J_tolerance': 1e-10,
               'beta_1': 0.5, 'beta_2': 0.95, 'rho': 0.9, 'max_amount_interpolation_points': 8, 'kernel_width': ep, 'advanced': False, 'draw_TR': False}

mu_list, J_FOM_list, J_kernel_list, FOCs, _ , times_FOM, global_counter = TR_Kernel(fom, kernel, parameter_space, global_counter, RKHS_norm, TR_parameters)